...
static ssize_t unwind_backtrace_common(const memory_t* memory,
    ...
    for (size_t index = 0; returned_frames < max_depth; index++) {
        ...
        add_backtrace_entry(pc, backtrace, ignore_depth, max_depth, &ignored_frames, &returned_frames);
    ...
    // Add a final entry for the LR if it looks sane and call it good.
      //LIBUDF_LOG("[Oliver_Zeng-1]: r_f %zu m_d %zu R_LR %zu R_PC %zu m_i_l %p\n", returned_frames, max_depth, state->gregs[R_LR], state->gregs[R_PC], map_info_list);
    if (returned_frames < max_depth
      ...
      //LIBUDF_LOG("[Oliver_Zeng-2] backtrace addr:%p\n", backtrace);
    }
